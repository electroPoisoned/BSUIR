...
public class Test {
    ...
    private static void createGUI() {
        ...
        JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, allFlightsScrollPane, searchResultsScrollPane);
    ...
    private static void addFlight() {
        ...
        int result = JOptionPane.showConfirmDialog(null, myPanel, "Add Flight", JOptionPane.OK_CANCEL_OPTION);
        if (result == JOptionPane.OK_OPTION) {
            Aeroflot aeroflot = new Aeroflot(destinationField.getText(), flightNumberField.getText(), (PlaneType) planeTypeComboBox.getSelectedItem());
    ...
    private static void saveFlights() {
        try {
            ...
            for (Aeroflot aeroflot : aeroflots) {
                writer.println(aeroflot.getDestination() + "," + aeroflot.getFlightNumber() + "," + aeroflot.getPlaneType());
            }
            writer.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static void sortFlights() {
        aeroflots.sort(Comparator.comparing(Aeroflot::getDestination));
        updateAllFlightsTable();
    }
    ...
    private static void searchFlights() {
        ...
        for (Aeroflot aeroflot : aeroflots) {
            if (aeroflot.getDestination().contains(search) || aeroflot.getFlightNumber().contains(search) || aeroflot.getPlaneType().toString().contains(search)) {
